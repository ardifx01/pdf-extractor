...
MODEL_PATH_YOLO = Path("./app/yolo/best.pt")
...
def extract_text_from_pdf_page(pdf_path, result_path, create_markdown, number_thread):
    ...
    try:
        with pymupdf.open(pdf_path) as pdf:
            ...
            for i, page in enumerate(pdf.pages()):
                ...
                image_path = temp_image_dir / f"{knowledge_id}_page_{page_index}.png"
                page_image.save(str(image_path))

                # YOLO inference
                results = model.predict(str(image_path), verbose=False, conf=0.7)
                boxes = []
                ...
                page_pdf_path = result_dir / f"{knowledge_id}_page_{page_index}.pdf"
                ...
                markdown_text = extract_text_from_pdf_page(
                    page_pdf_path,
                    result_dir / f"{knowledge_id}_page_{page_index}",
                    create_markdown,
                    number_thread,
                )
                ...
                yield log_info(
                    f"Processed page {page_index}/{pdf.page_count} of {knowledge_id}"
                )

                del mat, page_image, results, boxes, rectangles
                gc.collect()
